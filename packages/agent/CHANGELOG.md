...
- Tool calls flagged `providerExecuted` are no longer dispatched locally: the loop records a synthetic success result instead, so provider-side executions (Cursor's native tool calls) keep call/result pairing without re-running side effects or failing tool resolution. Unflagged calls, including Cursor's MCP and `todo_write` calls, still execute as before. The `streamProxy` wire protocol now relays the marker on `toolcall_start` and reconstructs it client-side, so provider-executed provenance survives proxy transport too.
